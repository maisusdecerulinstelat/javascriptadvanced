...
class Triangle {
  ...
  reportArea() {
    ...
    listItem.textContent = `${this.name}: arie este ${Math.round((Math.sqrt(3)/4)*(this.length * this.length))}px patrati.`
  ...
  reportPerimeter() {
    let listItem = document.createElement('li');
    listItem.textContent = `${this.name}: perimetrul este ${this.length * 3}px.`
